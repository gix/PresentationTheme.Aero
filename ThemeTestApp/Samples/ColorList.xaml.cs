namespace ThemeTestApp.Samples
{
    ...
    internal partial class ColorList : INotifyPropertyChanged
    {
        ...
        public ColorList()
        {
            ...
            Label = "Colors";
            Colors = new ObservableCollection<ColorSpec>();
            ...
            foreach (var property in typeof(SystemColors).GetProperties(BindingFlags.Public | BindingFlags.Static)) {
                if (property.PropertyType != typeof(Color))
                    continue;
                colors.Add(new ColorSpec(property.Name, (Color)property.GetValue(null)));
            }

            colors.Sort((l, r) => l.Name.CompareTo(r.Name));
            ...
            foreach (var property in typeof(ThemeColors).GetProperties(BindingFlags.Public | BindingFlags.Instance)) {
                if (property.PropertyType != typeof(Color))
                    continue;
                colors.Add(new ColorSpec(property.Name, (Color)property.GetValue(theme)));
            }
        ...
        public ObservableCollection<ColorSpec> Colors { get; private set; }
    ...
    internal sealed class ThemeColors : IDisposable
    {
        ...
        {
            theme = VisualStylesNativeMethods.OpenThemeData(hwnd, classList);
        }
        ...
        private Color GetThemeColor(CacheSlot slot)
        {
            lock (ColorCacheValid) {
                if (!ColorCacheValid[(int)slot]) {
                    uint bgra = (uint)FromWin32Value(VisualStylesNativeMethods.GetThemeSysColor(theme, SlotToFlag(slot)));
        ...
        public Color ActiveCaptionColor
        {
            get
            {
                return GetThemeColor(CacheSlot.ActiveCaption);
            }
        }
