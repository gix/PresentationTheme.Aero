...
typedef HRESULT (*PFNALLOCSECTIONS)(CUxThemeFile*, wchar_t*, unsigned, int, wchar_t*,
                                    unsigned, int, int);
...
public:
    ...
    HRESULT AddIndex(wchar_t const* pszAppName, wchar_t const* pszClassName, int iPartId,
                     int iStateId, int iIndex, int iLen) override;
    HRESULT AddData(short sTypeNum, unsigned char ePrimVal, void const* pData,
                    unsigned dwLen) override;
    ...
    BOOL IndexExists(wchar_t const* pszAppName, wchar_t const* pszClassName, int iPartId,
                     int iStateId);
    HRESULT AddMissingParent(wchar_t const* pszAppName, wchar_t const* pszClassName,
                             int iPartId, int iStateId);
    HRESULT AddDataInternal(short sTypeNum, unsigned char ePrimVal, void const* pData,
                            unsigned dwLen);
    void FreeLocalTheme();
    HRESULT LoadTheme(HINSTANCE hInst, wchar_t const* pszThemeName,
                      wchar_t const* pszColorParam, wchar_t const* pszSizeParam,
                      HANDLE* phReuseSection, bool fGlobalTheme, bool fHighContrast);
    HRESULT EmitString(MIXEDPTRS* u, wchar_t const* pszSrc, unsigned cchSrc,
                       int* piOffSet);
    HRESULT EmitObject(MIXEDPTRS* u, short propnum, unsigned char privnum, void* pHdr,
                       unsigned dwHdrLen, void* pObj, unsigned dwObjLen, CRenderObj*);
    ...
    HRESULT PackTextObjects(MIXEDPTRS* uOut, CRenderObj* pRender, int iMaxPart,
                            bool fGlobals);
    HRESULT PackTextObject(MIXEDPTRS* u, CRenderObj* pRender, int iPartId, int iStateId);
    HRESULT PackMetrics();
    HRESULT CopyDummyNonSharableDataToLive();
    HRESULT CreateReuseSection(wchar_t const* pszSharableSectionName,
    ...
    HRESULT CopyLocalThemeToLive(void* hFile, int iTotalLength,
    ...
    HRESULT PackDrawObjects(MIXEDPTRS* uOut, CRenderObj* pRender, int iMaxPart,
                            bool fGlobals);
    HRESULT CopyPartGroup(APPCLASSLOCAL* pac, MIXEDPTRS* u, int iPartId,
                          int* piPartJumpTable, int iPartZeroIndex, int iBaseClassIndex,
                          bool fGlobalsGroup);

    HRESULT AddIndexInternal(wchar_t const* pszAppName, wchar_t const* pszClassName,
