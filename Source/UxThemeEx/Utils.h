...
HRESULT SafeStringCchCopyW(wchar_t* pszDest, size_t cchDest,
...
{
    using ValueType = int;
    static int Acquire(HDC hdc, int mode)
    {
        return SetStretchBltMode(hdc, mode);
    }
    static void Release(HDC hdc, int oldMode)
    {
...
{
    using ValueType = T;
    static T Acquire(HDC hdc, T obj)
    {
...
#define TRACE_HR(hr)                                                           \
    (true ? ::uxtheme::TraceHResult((hr), __FILE__, __LINE__, __FUNCTION__)    \
          : (hr))
...
{
    return reinterpret_cast<BYTE*>(header) + header->biSize +
           4 * header->biClrUsed;
}
