...
typename details::unique_if<T>::unique_single make_unique_nothrow(Args&&... args)
{
    return std::unique_ptr<T>(new (std::nothrow) T(std::forward<Args>(args)...));
}

template<typename T>
typename details::unique_if<T>::unique_array_unknown_bound make_unique_nothrow(
    size_t size)
{
...
template<typename T, typename... Args>
typename details::unique_if<T>::unique_array_known_bound make_unique_nothrow(Args&&...) =
    delete;
...
template<typename T, typename... Args>
typename details::unique_if<T, FreeDeleter>::unique_single make_unique_malloc(size_t size)
{
...
template<typename T, typename... Args>
typename details::unique_if<T, FreeDeleter>::unique_array_known_bound make_unique_malloc(
    Args&&...) = delete;

template<typename T, typename... Args>
typename details::unique_if<T, FreeDeleter>::unique_single make_unique_calloc(size_t size)
{
...
template<typename T, typename... Args>
typename details::unique_if<T, FreeDeleter>::unique_array_known_bound make_unique_calloc(
    Args&&...) = delete;
...
template<typename T>
UXTHEME_ALWAYS_INLINE T Align8(T value)
{
    return AlignPower2<8>(value);
}
...
public:
    ...
    GdiScope(HDC hdc, T value)
        : hdc(hdc)
        , oldValue(Traits::Acquire(hdc, value))
    {}
...
template<>
inline bool SelectObjectTraits<HRGN>::IsValid(HRGN obj)
{
    return obj != HGDI_ERROR;
}
template<typename T>
...
inline HRESULT GetLastErrorAsHResult()
{
...
using THREADENUMPROC = BOOL(CALLBACK*)(DWORD ownerProcessId, DWORD threadId,
                                       LPARAM param);
...
struct VersionInfo
{
    ...
    friend bool operator==(VersionInfo const& lhs, VersionInfo const& rhs)
    {
